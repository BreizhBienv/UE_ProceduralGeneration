...
namespace CustomSimplexNoise
{
    ...
    static inline int32 FastFloor(float p)
    {
        int32 i = static_cast<int32>(p);
        return (p < i) ? (i - 1) : i;
    }

    static inline uint8 Hash(int32 p)
    {
        return permutationTable[static_cast<uint8>(p)];
    }
    ...
    *
    * @param[in] hash  value
    * @param[in] x     coord of the distance to the corner
    * @param[in] y     coord of the distance to the corner
    *
    * @return gradient value
    */
    static float Grad2D(int32_t pHash, float pX, float pY) {
        const int32_t h = pHash & 0x3F;  // Convert low 3 bits of hash code
    ...
    TArray<float> Map(const int32 pMapWidth, const int32 pMapHeight,
        float pScale, const FVector& pOrigin, 
